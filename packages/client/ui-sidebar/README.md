...
The expanded brand row renders `sidebar.brand.mark` and `sidebar.brand.name` as independent single slots, with an empty `[data-dsh-app-drag]` spacer between the brand and the collapse control so the desktop shell can map window drag to that handle only, while the collapsed rail renders the same mark slot. Without occupants, the shell uses the fish mark and a `DeepSeek Harness` label (`DSH_CLIENT_TITLE` when the build set one) carrying the build's 7-character `DSH_CLIENT_COMMIT_HASH` badge. A deployment package can replace either value without replacing the New Session control or rail geometry; declaration-aware `slots.inject()` lets such a package activate before or after the sidebar.
