...
外壳自带的唯一「通用」行是构建身份：**版本** 报告 `DSH_CLIENT_VERSION`，即构建期嵌入这批客户端产物的发布版本（[构建身份](../../../.agents/notes/implemented/feature/2026-08-21-client-version-in-settings.md)）。它排在最后，使版本号读起来像可修改偏好项下方的脚注；未经构建的源码运行显示 `开发构建`，而不是编造一个它并不具备的版本号。
...
- 「通用」分区不自带偏好行；除「版本」外的每一行仅在其所属功能插件挂载时出现。
