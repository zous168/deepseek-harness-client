...
| [`@resvg/resvg-wasm`](https://github.com/git@github.com:yisibl/resvg-js) | MPL-2.0 |
...
`@resvg/resvg-wasm` (MPL-2.0), `eslint-plugin-sonarjs` (LGPL-3.0-only) and `lightningcss` (MPL-2.0) run only as development tooling; their code is not linked into or distributed with any DeepSeek Harness artifact.
