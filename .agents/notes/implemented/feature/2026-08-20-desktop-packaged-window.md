...
`dsh desktop` launches `@deepseek-ai/dsh-desktop`, an Electron application that boots the existing `web` profile as a child and loads the printed loopback URL. The window title is `DeepSeek Harness`. The window is `frame: false`: there is no OS title bar. Drag is a property of the session header, sidebar logo row, blank-hero column, and title row — not of every descendant, and not of `#root` (an ancestor `no-drag` swallows descendant `drag` on Windows). The sidebar brand is re-marked `drag` so the product title moves the window; header action clusters, every other interactive control, and any `[role="dialog"]` / `[aria-modal="true"]` tree stay `no-drag`. Electron hit-tests `-webkit-app-region` independently of overlay paint, so an open `[aria-modal="true"]` also turns chrome drag off and hides the injected window-button cluster. The settings panel portals to `document.body` the same way `Modal` does, so sidebar `overflow` stacking cannot trap the dialog under that cluster. A leftover sibling drag strip is removed on inject. The shell injects minimize / maximize / close through a sandboxed preload. Session-header utilities (including Session log) clear that cluster via a `header[class*="header"]` padding rule that beats the conversation header's class padding. Edge resize stays on `thickFrame`. The window and electron-builder icon are the DeepSeek whale mark already used as the Web favicon, painted white on the brand-blue plate `#4D6BFE` at `apps/desktop/icons/icon.svg` and rasterized to `icons/icon.png` for installers.
