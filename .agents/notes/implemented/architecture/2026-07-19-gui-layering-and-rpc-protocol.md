...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` (`@deepseek-ai/dsh-desktop`) is the packaged Electron window: it boots the web profile and loads the printed loopback URL. The IPC fetch carrier remains unshipped ([packaged desktop window](../feature/2026-08-20-desktop-packaged-window.md)).
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Web (browser access) only; zero workspace dependencies (the registry arrives by structural injection); the shipped desktop window loads this loopback HTTP instead of replacing it with IPC |
...
The two existing applications preserve the division: the Web application mounts Host, carrier, and browser composition, while `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or ports. `dsh desktop` reuses that Web host inside a native window. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without fetch.
...
| IPC bridge subclass (hypothetical — the shipped desktop window loads loopback HTTP instead) | an Electron shell | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |
