...
`desktop:pack` deploys `@deepseek-ai/dsh` with `--legacy` and a hoisted linker, omits `--prod` so web-host packages the CLI also lists as devDependencies stay in the runtime, and does not set `link-workspace-packages` so workspace packages are copied rather than linked. It then asserts `runtime/lib/bin.js` and `runtime/node_modules/@deepseek-ai/dsh-app-boot` exist, points electron-builder at the already-unpacked Electron dist, and restores `package.json` after moving `electron` into `devDependencies` for the builder. electron-builder's extraResources FileMatcher drops `node_modules`, so `afterPack` copies the deployed tree (dereferenced) into the unpacked `resources/runtime` and re-asserts those boot paths. After deploy, pack also copies workspace packages that legacy hoisting or `link:` overrides omit (`@deepseek-ai/cosmokit` is the first of those). The packaged host starts that `runtime/lib/bin.js`. Pack also stamps `update-feed.json` from `DSH_DESKTOP_UPDATE_REPO`, `GITHUB_REPOSITORY`, or `origin` so the running window can query that repository's Releases ([update check](../feature/2026-08-20-desktop-update-check.md)).
